...
const UserPolicyCard = ({policy}) => {
    ...
    const handleFileClaim = () => { 
        navigate('/claim',{state:{policyId:policyId,userEmail:currentUser.email}})
    }
  return (
    <div>
       <Card className="mb-3">
                <Card.Body>
                  <Card.Title>Policy Id #{policy.policyNumber} | {findPolicy?.claim_type} | {findPolicy.policy_type} | Duration : {findPolicy.mount} |  
                    <Button variant="primary" className="ml-5" onClick={handleFileClaim}>
                File Claim
              </Button></Card.Title>
